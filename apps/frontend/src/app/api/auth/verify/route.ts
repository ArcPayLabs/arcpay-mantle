...
import { challengeCookie, createSession, readChallenge, sessionCookie, sessionTtlSeconds } from "@mantle/lib/server/session";

export async function POST(request: Request) {
  const body = await request.json();
  const challenge = readChallenge(request.headers.get("cookie")?.match(/arcpay_mantle_challenge=([^;]+)/)?.[1]);
  if (!challenge) {
  ...
  const { data: link, error } = await admin.auth.admin.generateLink({
    ...
    options: {
      data: {
        name: `Mantle ${recovered.slice(0, 6)}`,
