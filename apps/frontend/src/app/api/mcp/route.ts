...
import { developerTools, runDeveloperTool } from "@mantle/lib/server/developer-tools";
import { validateDeveloperKey } from "@mantle/lib/server/developer-keys";
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ok: true,
    service: "arcpay-mantle-remote-mcp",
  ...
  try {
    if (method === "initialize") {
      ...
      return jsonRpcResult(id, {
        protocolVersion: "2024-11-05",
        serverInfo: { name: "arcpay-mantle", version: "0.1.0" },
        capabilities: { tools: {} },
      });
    }
