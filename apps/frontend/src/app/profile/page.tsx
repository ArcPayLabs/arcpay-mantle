...
const EMPTY_PROFILE: ProfileForm = {
  ...
  notificationEmail: "",
  walletLabel: "Mantle operations wallet",
};
...
export default function ProfilePage() {
  ...
  const [status, setStatus] = useState("Connect a Mantle wallet or sign in with email to sync profile details.");
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setProfile({
        ...
        walletLabel: row?.wallet_label || "Mantle operations wallet",
      });
  ...
  async function connectWallet() {
    ...
    setStatus(walletAction.errorMessage ?? "Mantle wallet linked to this browser session.");
  }
    ...
    <ProductAppShell>
      <div className="space-y-6">
        <PageHeader
          ...
          title="Profile"
          description="Manage the operator profile, notification email, and Mantle wallet identity used for ArcPay treasury actions."
          actions={
            <button onClick={() => void connectWallet()} className="rounded-full bg-foreground px-4 py-2.5 text-sm font-semibold text-background">
              {walletAddress ? "Refresh wallet" : "Connect Mantle wallet"}
            </button>
          }
        />

        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <StatCard icon={Wallet} label="Wallet" value={walletAddress ? `${walletAddress.slice(0, 6)}...${walletAddress.slice(-4)}` : "--"} hint="Mantle signer" />
