...
export const MANTLE_CHAIN_ID_HEX = "0x138b";
export const MANTLE_RPC_URL = "https://rpc.sepolia.mantle.xyz";
export const MANTLE_EXPLORER_URL = "https://sepolia.mantlescan.xyz";
...
export const CONTRACTS = {
  ...
  MantleAgentRiskOracle: deployedContracts.MantleAgentRiskOracle ?? NATIVE_TOKEN,
...
export const MANTLE_AGENT_PLATFORM_ABI = [
...
export function publicProvider() {
  return new JsonRpcProvider(MANTLE_RPC_URL, MANTLE_CHAIN_ID);
}

export async function riskOracleQuote() {
  const provider = publicProvider();
  const oracle = new Contract(CONTRACTS.MantleAgentRiskOracle, RISK_ORACLE_ABI, provider);
  const platform = new Contract(MANTLE_AGENT_PLATFORM_ADDRESS, MANTLE_AGENT_PLATFORM_ABI, provider);
  ...
  return {
    agentId: MANTLE_RISK_AGENT_ID,
    platform: MANTLE_AGENT_PLATFORM_ADDRESS,
    pricePerAgentWei,
    requestDeposit,
    subcommitteeSize,
...
export async function switchToMantle() {
  ...
  try {
    await window.ethereum.request?.({
      ...
      params: [{ chainId: MANTLE_CHAIN_ID_HEX }],
    });
  } catch {
    await window.ethereum.request?.({
      ...
      params: [{
        ...
        nativeCurrency: { name: "Mantle Testnet Token", symbol: "MNT", decimals: 18 },
...
export async function riskOracleContract() {
  const signer = await signerProvider();
  return new Contract(CONTRACTS.MantleAgentRiskOracle, RISK_ORACLE_ABI, signer);
}
...
export async function erc20Contract(address = USDY_TOKEN_ADDRESS) {
  const signer = await signerProvider();
...
export async function privacyVaultContract() {
  const signer = await signerProvider();
  return new Contract(CONTRACTS.MantlePrivacyVault, PRIVACY_VAULT_ABI, signer);
}
...
export function readRecords(): LocalRecord[] {
  ...
  return JSON.parse(window.localStorage.getItem("arcpay-mantle-records") ?? "[]") as LocalRecord[];
}
...
export function writeRecord(record: Omit<LocalRecord, "createdAt">) {
  ...
  window.localStorage.setItem("arcpay-mantle-records", JSON.stringify(next));
  fetch("/api/records", {
    method: "POST",
