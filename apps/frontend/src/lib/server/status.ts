...
const X402_URL = process.env.NEXT_PUBLIC_X402_SERVER_URL ?? "https://mantle-x402.20.208.46.195.nip.io";
const EXPLORER_URL = "https://sepolia.mantlescan.xyz";
const RECORDS_TABLE = validRecordsTable(process.env.ARCPAY_RECORDS_TABLE ?? "arcpay_mantle_records");
...
export async function collectRuntimeStatus(origin?: string): Promise<RuntimeStatus> {
  ...
  return {
    ...
    network: {
      name: String((deployment as { network?: string }).network ?? "mantle-testnet"),
      chainId: Number((deployment as { chainId?: number }).chainId ?? 5003),
      rpcUrl: RPC_URL,
...
async function checkRpc(): Promise<RuntimeCheck> {
  ...
  try {
    ...
    return check("Mantle RPC", chainId === expected ? "ok" : "degraded", `Block ${blockNumber}`, `chain ${chainId}, ${Date.now() - startedAt}ms`);
  } catch (error) {
    return check("Mantle RPC", "down", "RPC check failed", errorMessage(error));
  }
}
...
async function checkContracts(): Promise<RuntimeCheck> {
  try {
    ...
    return check("Mantle contracts", "down", "Contract bytecode check failed", errorMessage(error));
  }
}
...
async function checkX402(): Promise<RuntimeCheck> {
  try {
    ...
    return check("x402 gateway", body?.service === "arcpay-mantle-x402" ? "ok" : "degraded", "HTTP 402 payment gateway reachable", trim(JSON.stringify(body), 180));
  } catch (error) {
...
async function checkDocs(origin?: string): Promise<RuntimeCheck> {
  const base = origin || process.env.NEXT_PUBLIC_APP_URL || "https://arcpay-mantle.vercel.app";
  try {
...
function validRecordsTable(table: string) {
  return /^arcpay_[a-z0-9_]+_records$/.test(table) ? table : "arcpay_mantle_records";
}
