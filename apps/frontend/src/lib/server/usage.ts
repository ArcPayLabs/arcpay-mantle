const TABLE = "arcpay_mantle_usage_events";
...
export async function readUsageAnalytics() {
  ...
  const [events, recent, beta, developerKeys, records] = await Promise.all([
    ...
    fetchJson<Array<{ status: string; wallet_address: string | null; agent_url: string | null }>>("arcpay_mantle_beta_signups?select=status,wallet_address,agent_url&limit=1000"),
    fetchJson<Array<{ owner: string; revoked_at: string | null; last_used_at: string | null }>>("arcpay_mantle_developer_keys?select=owner,revoked_at,last_used_at&limit=1000"),
    fetchJson<Array<{ type: string; owner: string; tx_hash: string | null; created_at: string }>>("arcpay_mantle_records?select=type,owner,tx_hash,created_at&order=created_at.desc&limit=1000"),
  ]);
