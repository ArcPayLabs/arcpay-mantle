...
import { agentIdFromSlug, fromWei, registryContract, shortAddress, toWei, writeRecord } from "@mantle/lib/mantle";
...
function AgentsRoute() {
  ...
  const [status, setStatus] = useState("Register an agent so other Mantle agents can discover and hire it.");
  ...
  const protectedUrl = `https://mantle-x402.20.208.46.195.nip.io/agent/${encodeURIComponent(form.slug)}/work`;
  ...
  async function registerAgent() {
    ...
    setStatus(`Agent registered on Mantle: ${tx.hash}`);
    await loadAgent();
  }
  ...
  async function loadAgent() {
    ...
    setStatus("Agent loaded from Mantle registry.");
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Agents"
        description="Publish agent capability metadata, endpoints, and MNT pricing into the Mantle registry so autonomous clients can discover and hire services."
      />

      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <StatCard icon={Bot} label="Registry" value="Live" hint="Mantle contract" />
        <StatCard icon={Sparkles} label="Price" value={`${form.price} MNT`} hint="Per service call" />
      ...
      <section className="min-w-0 overflow-hidden rounded-3xl border border-border bg-card p-5 md:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-4 text-2xl font-semibold tracking-tight">Publish any Mantle-ready service endpoint.</h2>
          ...
          <a href="https://docs.mantle.network/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-border bg-background px-4 py-2.5 text-sm font-semibold">
            Mantle docs <ExternalLink className="h-4 w-4" />
          </a>
        </div>
          ...
          <div className="min-w-0 rounded-2xl border border-border bg-background p-4">
            ...
            <div className="mt-4 space-y-3">
              {[
                ...
                "Register the slug, capabilities, endpoint, and MNT price on Mantle.",
        ...
        <section className="rounded-3xl border border-border bg-card p-5 md:p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <button onClick={() => void registerAgent()} className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground">
              <Plus className="h-4 w-4" /> Register on Mantle
            </button>
          ...
          {agent ? (
            ...
            <div className="mt-5">
              <EmptyState
                ...
                description="Register a Mantle service agent or load an existing slug to inspect its endpoint, price, capabilities, and registry owner."
...
const AGENT_TEMPLATES = [
  ...
  },
  {
    ...
    endpoint: "https://example.com/mantle-agent/routing",
    capabilities: "real-time-routing, policy-checks, x402-quotes",
    price: "0.005",
    description: "Mantle-speed payment coordinator for route intents, x402 quotes, and policy-safe execution plans.",
  },
  {
    slug: "privacy-agent",
    name: "Privacy Intent Agent",
    endpoint: "https://example.com/mantle-agent/privacy",
...
function templateSnippet(slug: string) {
  return [
    `const res = await fetch("https://mantle-x402.20.208.46.195.nip.io/agent/${encodeURIComponent(slug)}/work");`,
    "if (res.status === 402) {",
