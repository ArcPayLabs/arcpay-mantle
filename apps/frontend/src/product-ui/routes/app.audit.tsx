...
import { fetchRecords, readRecords, shortAddress, writeRecord, type LocalRecord } from "@mantle/lib/mantle";
...
function AuditPage() {
  ...
  const [message, setMessage] = useState("Audit records are sourced from Mantle actions, local evidence, and the Supabase mirror.");
  ...
  function createViewingKey() {
    ...
    writeRecord({
      id,
      type: "viewing-key",
      title: "Scoped auditor access for Mantle treasury",
      status: "configured",
    });
  ...
  function exportCsv() {
    ...
    link.href = url;
    link.download = "arcpay-mantle-audit.csv";
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
    setMessage(`Exported ${rows.length} Mantle audit rows.`);
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Audit"
        description="A clean audit trail for Mantle treasury actions, agent orders, policies, payment intents, privacy commitments, and operator approvals."
        actions={
          <>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Audit rows" value={records.length} hint="Mantle local + server records" />
