...
import { connectedAddress, hashText, shortAddress, writeRecord } from "@mantle/lib/mantle";
...
const STORAGE_KEY = "arcpay-mantle-contractors";
...
function ContractorsPage() {
  ...
  const [message, setMessage] = useState("Add Mantle contractors, risk-score them, and prepare wallet-signed payout batches.");
  ...
  function addContractor() {
    if (!form.name.trim() || !form.wallet.trim()) {
      setMessage("Name and Mantle wallet are required.");
      return;
    }
    ...
    setOpen(false);
    setMessage(`${contractor.name} added with deterministic Mantle risk score ${riskScore}.`);
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Contractors"
        description="Maintain contractor allowlists, Mantle wallet identities, risk scores, and batch payout intents before any treasury action is signed."
        actions={
          <>
