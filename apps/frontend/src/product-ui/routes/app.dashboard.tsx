...
import { CONTRACTS, balances, connectedAddress, fetchRecords, shortAddress, type LocalRecord } from "@mantle/lib/mantle";
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-primary/30 bg-primary/5 p-5 flex flex-wrap items-center gap-4">
        ...
        <div className="flex-1 min-w-[240px]">
          ...
          <div className="text-sm text-muted-foreground">
            {wallet ? `${shortAddress(wallet)} has ${Number(balance || 0).toFixed(4)} MNT. Register an agent, set policy, then create an x402 or escrow order.` : "Connect a Mantle wallet to load live balance and start the agent treasury flow."}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <section className="rounded-2xl border border-border bg-card p-5">
          ...
          <h2 className="mt-1 text-lg font-medium">Mantle contract map</h2>
