...
function InvoicesPage() {
  ...
  const tokenOptions = TOKENS_BY_NETWORK[network] ?? TOKENS_BY_NETWORK.mantle;
  const [items, setItems] = useState<Invoice[]>([]);
  const [open, setOpen] = useState(false);
  const [review, setReview] = useState<Form | null>(null);
  const [copied, setCopied] = useState<string | null>(null);
  const [message, setMessage] = useState("Connect a Mantle wallet to create or pay invoices.");
  const [loading, setLoading] = useState(false);
  const [busyId, setBusyId] = useState<string | null>(null);

  const { register, handleSubmit, reset, formState: { errors } } = useForm<Form>({
    resolver: zodResolver(schema),
    defaultValues: { token: "MNT", payer: "" },
  });
  ...
  async function loadInvoices() {
    ...
    setMessage("Invoices loaded. On-chain status can be refreshed from Mantle.");
  }
  ...
  const confirm = async () => {
    ...
    setMessage(`Invoice ${publicId} created on Mantle.`);
    setReview(null);
    setOpen(false);
  ...
  async function payInvoice(invoice: Invoice) {
    ...
    try {
      ...
      setMessage(`Invoice ${invoice.publicId} paid on Mantle.`);
    } catch (error) {
  ...
  async function refreshOnchain(invoice: Invoice) {
    ...
    try {
      ...
      setMessage(`Invoice ${invoice.publicId} is ${status} on Mantle.`);
    } catch (error) {
      setMessage(error?.shortMessage || error?.message || String(error));
    } finally {
      setBusyId(null);
...
function rowToInvoice(row: any): Invoice {
  ...
  return {
    ...
    amount,
    token,
    amountUnits: row.amount_units ?? (token === "MNT" ? toWei(String(amount)).toString() : toUnits(String(amount), 6).toString()),
    due: row.due ?? "",
    memo: row.memo ?? "",
...
function readLocalInvoices(): Invoice[] {
  ...
  try {
    return JSON.parse(window.localStorage.getItem("arcpay-mantle-invoices") ?? "[]");
  } catch {
    return [];
  }
}
...
function writeLocalInvoices(items: Invoice[]) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem("arcpay-mantle-invoices", JSON.stringify(items.slice(0, 100)));
}
