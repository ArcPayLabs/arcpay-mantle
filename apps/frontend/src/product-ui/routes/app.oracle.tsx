...
import { fromWei, hashText, riskOracleContract, riskOracleQuote, shortAddress, txUrl, writeRecord } from "@mantle/lib/mantle";
...
function OracleRoute() {
  ...
  useEffect(() => {
    riskOracleQuote()
      .then((next) => {
        setQuote(next);
        setStatus(`Mantle Agent requester deposit loaded: ${fromWei(next.totalWei)} MNT total for agent ${next.agentId}.`);
      })
      .catch((error) => setStatus(error instanceof Error ? error.message : "Unable to load Mantle agent requester quote."));
  }, []);
  ...
  async function requestRisk() {
    ...
    setResult(null);
    writeRecord({ id: crypto.randomUUID(), type: "oracle", title: "Requested Mantle agent risk score", status: "requested", amount: fromWei(nextQuote.totalWei), txHash: tx.hash });
    setStatus(`Risk oracle requested through Mantle Agent platform: ${tx.hash}`);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <StatCard icon={Gauge} label="Oracle" value="Live" hint="Mantle testnet contract" />
        <StatCard icon={Sparkles} label="Agent request fee" value={quote ? `${fromWei(quote.totalWei)} MNT` : "--"} hint="Live platform deposit + agent budget" />
        <StatCard label="Last request" value={requestId || "--"} hint="Mantle agent request id" emphasis />
      </div>
      {quote && (
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <StatCard label="Platform" value={shortAddress(quote.platform)} hint="Mantle requester contract" />
          <StatCard label="Risk agent" value={quote.agentId} hint="Configured agent id" />
          <StatCard label="Subcommittee" value={quote.subcommitteeSize.toString()} hint={`${fromWei(quote.pricePerAgentWei)} MNT per agent`} />
        </div>
      )}
