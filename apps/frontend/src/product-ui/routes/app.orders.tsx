...
import { agentIdFromSlug, fromWei, orderBookContract, shortAddress, toWei, writeRecord } from "@mantle/lib/mantle";
...
function OrdersRoute() {
  ...
  const [status, setStatus] = useState("Create and reconcile agent orders through the Mantle order book.");

  async function createOrder() {
    setStatus("Creating order on Mantle...");
  ...
  async function loadOrder() {
    ...
    setStatus("Order loaded from Mantle.");
  }

  return (
    <div className="space-y-6">
      <PageHeader icon={Workflow} eyebrow="Order state machine" title="Orders" description="Create, load, and reconcile Mantle agent orders with escrowed MNT and state-machine evidence." />
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <StatCard icon={Workflow} label="Lifecycle" value="7 states" hint="Pending to failed" />
        <StatCard label="Escrow" value={`${form.amount} MNT`} hint="Native value" />
      ...
      <section className="rounded-3xl border border-border bg-card p-5 md:p-6">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Amount MNT"><input className="ap-in" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} /></Field>
    ...
    <section className="rounded-3xl border border-border bg-card p-5 md:p-6">
      {record ? <div className="grid gap-3 md:grid-cols-2">{Object.entries(record).map(([key, value]) => <div key={key} className="rounded-2xl bg-muted/50 p-3"><div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{key}</div><div className="mt-1 truncate font-mono text-sm">{value.startsWith("0x") ? shortAddress(value) : value}</div></div>)}</div> : (
        <EmptyState
          ...
          description="Create an escrowed agent order, or paste an existing order id to load state-machine evidence from the Mantle contract."
        />
      )}
    </section>
  );
