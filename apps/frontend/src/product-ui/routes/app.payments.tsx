...
const STORAGE_KEY = "arcpay-mantle-payments";
...
function PaymentsPage() {
  ...
  const [message, setMessage] = useState("Create signer-gated payment intents for Mantle agents and operators.");
  const [form, setForm] = useState({
    recipient: "",
    amount: "0.01",
    token: "MNT" as PaymentRequest["token"],
  ...
  async function confirmPayment() {
    ...
    setMessage(`Payment intent signed by ${shortAddress(signer)}. Submit through the linked Mantle order, card, or treasury contract.`);
    setReview(null);
    setOpen(false);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label="Network" value="5003" hint="Mantle Testnet only" emphasis />
      </div>
      ...
      <ReviewModal
        ...
        title="Sign Mantle payment intent"
        description="This confirms the operator intent. Contract execution still requires the relevant Mantle transaction signature."
        rows={rows}
