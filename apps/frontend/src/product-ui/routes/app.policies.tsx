  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card title="Risk floor" desc="Minimum Mantle risk score required for any counterparty.">
          <Slider label="Minimum Mantle score" value={minScore} onChange={onChange(setMinScore)} max={100} step={1} format={(v) => `${v} / 100`} />
        ...
        <Card title="Allowed networks" desc="Lock movement to Mantle Testnet for this build. All actions stay wallet-signer gated.">
          <div className="flex flex-wrap gap-2">
            {NETWORKS.map((n) => (
        ...
        <Card title="Contractor allowlist" desc="Only these wallets can be paid via batch payroll.">
          <div className="flex gap-2">
            <input value={newAddr} onChange={(e) => setNewAddr(e.target.value)} placeholder="Mantle EVM address" className="flex-1 px-3 py-2 rounded-xl border border-border bg-background text-sm font-mono outline-none focus:ring-2 focus:ring-ring" />
