...
import { shortAddress, writeRecord } from "@mantle/lib/mantle";
...
const STORAGE_KEY = "arcpay-mantle-risk";
...
function RiskPage() {
  ...
  const [message, setMessage] = useState("Score Mantle counterparties before payments, card funding, and agent order execution.");
  ...
  function runLookup() {
    ...
    if (!wallet) return;
    const result = scoreMantleWallet(wallet, minScore);
    ...
    setMessage(`Mantle risk score generated for ${shortAddress(wallet)}.`);
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Risk"
        description="Score a Mantle counterparty wallet with the agent risk model before allowing treasury payments, cards, or order settlement."
      />
      ...
      <section className="rounded-3xl bg-surface-dark p-6 text-surface-dark-foreground">
        ...
        <h2 className="mt-1 text-3xl font-medium tracking-tight">Score any Mantle wallet</h2>
        <p className="mt-2 max-w-2xl text-sm text-white/60">
          This testnet model uses deterministic wallet features, local activity count, and policy floor. It is built to be swapped into a live Mantle oracle as ecosystem risk feeds mature.
        </p>
        <div className="mt-5 flex flex-col gap-2 sm:flex-row">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Paste a Mantle EVM address"
        ...
        {lookup && (
          <div className="mt-5 grid grid-cols-1 gap-3 lg:grid-cols-3">
            <div className="rounded-2xl bg-white/5 p-5">
              <div className="text-xs text-white/50">Mantle score</div>
          ...
          {history.length === 0 ? (
            <div className="p-5">
              <EmptyState
                ...
                description="Paste a Mantle address to generate a policy-aware counterparty score before payments, cards, invoices, or order settlement."
              />
            </div>
          ) : null}
          {history.map((item) => (
...
function scoreMantleWallet(wallet: string, minScore: number): RiskLookup {
  let seed = 0;
  ...
  return {
    ...
    recommendation,
    reasons: [
      `Deterministic Mantle score calculated from wallet entropy and observed action model.`,
