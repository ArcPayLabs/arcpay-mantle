...
type IntegrationKey = "mantleRpc" | "agentRegistry" | "orderBook" | "policyEngine" | "spendCards" | "privacyVault" | "riskOracle" | "mcp";

const INTEGRATIONS: { key: IntegrationKey; label: string; description: string }[] = [
  { key: "mantleRpc", label: "Mantle RPC", description: "Testnet reads, balances, and transaction checks" },
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (cached.defaultNetwork === "mantle") setNetwork(cached.defaultNetwork);
      }
    } catch {
    ...
    async function loadSettings() {
      ...
      const workspaces = await loadWorkspaces(supabase, NETWORK, account?.workspaceName ?? "Mantle agent treasury");
      if (!mounted) return;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <section className="lg:col-span-2 rounded-2xl border border-border bg-card p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-5">
            ...
            <Toggle label="Risk alerts" description="Notify when Mantle risk score drops below policy." enabled={riskAlerts} onToggle={() => setRiskAlerts((v) => !v)} icon={ShieldCheck} />
        ...
        <section className="rounded-2xl border border-border bg-card p-5">
          ...
          <div className="text-xs text-muted-foreground mt-0.5 mb-4">This deployment is fixed to Mantle Testnet so workspace policy, signing, and contract reads stay deterministic.</div>
          ...
          <div className="mt-4 rounded-xl bg-muted p-3 text-xs text-muted-foreground">
            Current mode: <span className="font-semibold text-foreground">{network}</span>. All actions require wallet review and Mantle Testnet signing.
          </div>
        </section>
