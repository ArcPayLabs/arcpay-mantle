...
import { agentIdFromSlug, orderBookContract, shortAddress, writeRecord } from "@mantle/lib/mantle";
...
function X402Route() {
  const [form, setForm] = useState({
    serverUrl: process.env.NEXT_PUBLIC_X402_SERVER_URL || "https://mantle-x402.20.208.46.195.nip.io",
    agentSlug: "research-agent",
    orderId: "",
    resultUri: "ipfs://arcpay-x402-result/research-agent",
    adminSecret: "",
  });
  const [status, setStatus] = useState("Quote an x402 protected Mantle agent endpoint, pay through the order book, verify, fulfill, and unlock work.");
  ...
  async function quoteEndpoint() {
    ...
    try {
      ...
      setQuote(body);
      setStatus(`Quote ready: ${body.accepts[0].amountStt} MNT for ${body.agent.name || form.agentSlug}.`);
    } catch (error) {
  ...
  async function payOrder() {
    ...
    setBusy(true);
    setStatus("Creating escrowed Mantle order from wallet...");
    try {
      ...
      writeRecord({ id: crypto.randomUUID(), type: "x402", title: `Paid x402 order for ${form.agentSlug}`, status: "paid", amount: `${accept.amountStt} MNT`, txHash: tx.hash });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="x402"
        description="Expose paid Mantle agent work over HTTP 402: quote requirements, escrow MNT on-chain, verify order status, fulfill, and unlock the protected result."
        actions={
        ...
        <Panel title="Payment requirement">
          {quote ? (
            ...
            <EmptyState
              ...
              description="Quote the protected endpoint to return exact Mantle payment requirements before creating an escrowed order."
