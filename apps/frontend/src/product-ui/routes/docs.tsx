...
const MODULES = [
  { icon: Bot, title: "Agent registry", body: "Publish agent capability metadata, endpoint URLs, MNT pricing, and active status on Mantle.", href: "/app/agents" },
  { icon: Workflow, title: "Order book", body: "Escrow MNT for autonomous work and reconcile pending, processing, fulfilled, settled, refunded, or failed states.", href: "/app/orders" },
  ...
  { icon: CreditCard, title: "USDY cards", body: "Give agents bounded token budgets with top-up, freeze, activate, and spend controls.", href: "/app/cards" },
  ...
  { icon: Gauge, title: "Risk oracle", body: "Quote the live Mantle agent requester deposit and store risk callback evidence for operator review.", href: "/app/oracle" },
  ...
  { icon: Terminal, title: "Published CLI", body: "Install @arcpaylabs/mantle-cli to derive IDs, print contracts, generate MCP config, and inspect guides.", href: "/proofs" },
...
const QUICKSTART = [
  "Connect an EVM wallet and switch to Mantle Testnet.",
      ...
      <main className="px-6 pb-24 pt-16">
        <div className="mx-auto max-w-[88rem]">
          <SectionHeading
            align="center"
            eyebrow="Docs"
            title={<>Operate an <span className="text-primary">agent treasury</span> on Mantle.</>}
          ...
          <section className="mt-12 grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="rounded-[2rem] border border-border bg-card p-6 md:p-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold tracking-tight">Mantle runtime</h2>
                  <p className="text-sm text-muted-foreground">Fixed to testnet for predictable operator flows.</p>
                </div>
              </div>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <Info label="Network" value="Mantle Testnet" />
                <Info label="Chain ID" value="5003 / 0x138b" />
                <Info label="Gas" value="MNT" />
                <Info label="x402" value="https://mantle-x402.20.208.46.195.nip.io" mono />
              </div>
              <div className="mt-6 flex flex-wrap gap-2">
                ...
                <a href="https://sepolia.mantlescan.xyz" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-semibold">
            ...
            <div className="overflow-hidden rounded-[2rem] border border-border bg-card">
              ...
              <div className="max-h-[29rem] divide-y divide-border overflow-auto">
                {contracts.map(([name, address]) => (
                  <a key={name} href={`https://sepolia.mantlescan.xyz/address/${address}`} target="_blank" rel="noreferrer" className="grid gap-2 px-6 py-4 text-sm hover:bg-muted/50 md:grid-cols-[220px_1fr_24px] md:items-center">
