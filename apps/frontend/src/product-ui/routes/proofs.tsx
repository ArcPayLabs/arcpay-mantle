...
export const Route = createFileRoute("/proofs")({
  head: () => ({
    meta: [
      { title: "Trust Center - ArcPay Mantle" },
      { name: "description", content: "Mantle contract deployment, worker, MCP, CLI, and frontend verification evidence." },
    ],
  }),
...
const CHECKS: { surface: string; status: Status; evidence: string }[] = [
  ...
  { surface: "Worker", status: "live", evidence: "Azure systemd worker listens to Mantle registry, policy, treasury, order, card, operator, privacy, invoice, risk, and reputation events from the deployed contracts." },
  ...
  { surface: "MCP, CLI, starter kit", status: "live", evidence: "Published npm packages: @arcpaylabs/mantle-cli, @arcpaylabs/mantle-mcp, and @arcpaylabs/mantle-x402-agent-starter." },
];
      ...
      <div className="px-6 pb-24 pt-16">
        <div className="mx-auto max-w-[88rem]">
          <SectionHeading
            ...
            description="ArcPay publishes its Mantle contracts, verification commands, worker surface, x402 server, and agent tooling so operators can inspect the stack before relying on it."
          />
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <section className="rounded-3xl border border-border bg-card p-6">
              ...
              <div className="mt-6 rounded-2xl bg-muted p-4 text-sm text-muted-foreground">
                Network: Mantle Testnet, chain {String((deployment as { chainId: number }).chainId)}. Currency: MNT.
              </div>
            </section>
          </div>
