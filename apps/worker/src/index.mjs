...
const deployment = JSON.parse(fs.readFileSync(path.join(root, "deployments", "mantle-testnet.json"), "utf8"));
...
const checkpointPath = process.env.ARCPAY_WORKER_CHECKPOINT_PATH ?? path.join(root, ".arcpay-mantle-worker-checkpoint.json");
const recordsTable = process.env.ARCPAY_RECORDS_TABLE ?? "arcpay_mantle_records";
...
const fallbackOwner = process.env.ARCPAY_WORKER_FALLBACK_OWNER ?? "mantle-worker";
...
const sources = [
  ...
  { key: "privacy", address: deployment.contracts.MantlePrivacyVault, abi: privacyAbi, contractName: "MantlePrivacyVault" },
  { key: "invoice", address: deployment.contracts.AgentInvoiceBook, abi: invoiceAbi, contractName: "AgentInvoiceBook" },
  { key: "risk", address: deployment.contracts.MantleAgentRiskOracle, abi: riskAbi, contractName: "MantleAgentRiskOracle" },
...
async function agentRecord(contract, base) {
  ...
  return {
    ...
    amount: priceWei !== undefined ? `${formatEther(priceWei)} MNT` : undefined,
  };
}
...
function policyRecord(base) {
  ...
  return {
    ...
    status,
    amount: amountWei !== undefined ? `${formatEther(amountWei)} MNT` : undefined,
  };
}
