...
const ARCPAY_X402_SCHEME = "arcpay-mantle-escrow-v1";
...
export function createX402Server(options = {}) {
  ...
  const rpcUrl = options.rpcUrl || process.env.MANTLE_RPC_URL || "https://rpc.sepolia.mantle.xyz";
  const provider = new JsonRpcProvider(rpcUrl, deployment.chainId || 5003);
...
async function route(ctx) {
  ...
  if (parts[0] === "agent" && parts[1] && parts[2] === "work" && req.method === "GET") {
    ...
    return sendJson(res, 200, {
      ...
      agentSlug: slug,
      result: {
        summary: `Paid Mantle x402 work unlocked for ${slug}.`,
  ...
  if (parts[0] === "agent" && parts[1] && parts[2] === "provider" && parts[3] === "fulfill" && req.method === "POST") {
    ...
    const result = await providerFulfill({
      orderBookSigner,
      orderId: body.orderId,
      resultUri: body.resultUri || `x402://arcpay-mantle/${parts[1]}/${body.orderId}`,
    });
    void trackUsageEvent({
...
async function paymentRequirements({ slug, req, registry, deployment }) {
  ...
  return {
    ...
    accepts: [{
      ...
      description: `Pay ${agent.name || slug} for protected Mantle agent work.`,
      resource: requestUri,
...
function demoPayload(deployment) {
  return {
    ok: true,
    title: "ArcPay Mantle x402 demo",
    steps: [
      "Register an agent in AgentRegistry with a public endpoint.",
      "Call GET /agent/:slug/work and receive HTTP 402 with exact Mantle payment requirements.",
...
function sendPaymentRequired(res, requirements, extra = {}) {
  res.setHeader("X-Accept-Payment", "arcpay-mantle-x402");
...
function caip2Network(deployment) {
  return `eip155:${deployment.chainId || 5003}`;
}
...
async function trackUsageEvent(input) {
  ...
  try {
    await fetch(`${process.env.SUPABASE_URL}/rest/v1/arcpay_mantle_usage_events`, {
      method: "POST",
      headers: {
        apikey: key,
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  createX402Server().listen(port, () => {
    console.log(`ArcPay Mantle x402 server listening on http://127.0.0.1:${port}`);
  });
}
