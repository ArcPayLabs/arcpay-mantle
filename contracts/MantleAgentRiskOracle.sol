...
interface IMantleAgentRequester {
...
contract MantleAgentRiskOracle {
    struct RiskResult {
        uint256 requestId;
        bytes32 orderId;
        address requester;
    ...
    function requestRisk(bytes32 orderId, string calldata prompt) external payable returns (uint256 requestId) {
        ...
        uint256 deposit = IMantleAgentRequester(platform).getRequestDeposit() + (pricePerAgentWei * subcommitteeSize);
        require(msg.value >= deposit, "underfunded");

        bytes memory payload = abi.encode(prompt);
        requestId = IMantleAgentRequester(platform).createRequest{value: deposit}(
            riskAgentId,
            address(this),
        ...
        onlyPlatform
    {
        ...
        string memory evidenceUri = "mantle-agent://no-result";
        if (status == AgentResponseStatus.Success && responses.length > 0) {
            ...
            if (raw.length >= 32) {
                ...
                evidenceUri = "mantle-agent://risk-score";
            } else if (raw.length > 0) {
                verdict = string(raw);
                evidenceUri = "mantle-agent://risk-verdict";
            }
