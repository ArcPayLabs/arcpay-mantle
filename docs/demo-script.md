...
1. Start on the landing page.
   - Say: "ArcPay turns Mantle Testnet into an agent treasury workspace. It is fixed to chain 5003, so every transaction in this demo is verifiable on the same explorer."
...
2. Open the app and connect the wallet.
   ...
   - Say: "The operator signs in with an EVM wallet. The workspace stores user records in Supabase, while critical agent, policy, order, card, invoice, and privacy actions land on Mantle."

3. Register an agent.
   - Open `Agents`.
   - Register or load `research-agent`.
   - Say: "The agent registry publishes a service endpoint, capability tags, and MNT price. Other agents or clients can discover the service and know what it costs before calling it."
...
5. Show live x402.
   - Open `x402`.
   - Use `https://mantle-x402.20.208.46.195.nip.io` as the server URL.
   - Quote the resource, create the order, verify it, fulfill it, then unlock the resource.
   - Say: "This is the agent commerce loop: the endpoint returns HTTP 402, the requester pays on Mantle, the provider fulfills the order, and the resource unlocks only after on-chain verification."
...
"ArcPay makes Mantle useful for real agent businesses: discover agents, hire them, enforce policy, manage spend, preserve private context, and prove every financial action."
