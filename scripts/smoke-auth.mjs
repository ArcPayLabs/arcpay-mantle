...
async function smokeWalletWorkspace() {
  ...
  const link = await admin.auth.admin.generateLink({
    ...
    options: {
      data: {
        name: `Mantle ${wallet.address.slice(0, 6)}`,
        wallet_address: wallet.address,
        workspace: "Mantle agent treasury",
      },
    },
  });
  ...
  await upsertWorkspace(verified.data.user.id, `Mantle ${wallet.address.slice(0, 6)}`, "", wallet.address, "Mantle agent treasury");
  pass("Wallet auth + workspace", `session/profile/workspace ok for ${wallet.address.slice(0, 6)}...${wallet.address.slice(-4)}`);
}

async function smokeMantleReads() {
  ...
  pass("Mantle RPC + contract reads", `chain 5003 block ${await provider.getBlockNumber()}`);
}
