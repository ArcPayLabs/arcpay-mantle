...
if (!process.env.PRIVATE_KEY) {
  throw new Error("PRIVATE_KEY missing. Funded Mantle smoke requires a testnet signer.");
}
...
const usdy = new Contract(deployment.usdyToken, [
...
const oracle = new Contract(c.MantleAgentRiskOracle, [
...
const platform = new Contract(deployment.mantleAgentPlatform, ["function getRequestDeposit() view returns (uint256)"], provider);
...
await run("Signer and Mantle funds", async () => {
  const network = await provider.getNetwork();
  if (Number(network.chainId) !== 5003) throw new Error(`wrong chain ${network.chainId}`);
  const balance = await provider.getBalance(deployer.address);
  if (balance < parseEther("1")) throw new Error(`low MNT balance ${formatEther(balance)}`);
  return `${deployer.address.slice(0, 6)}...${deployer.address.slice(-4)} has ${formatEther(balance)} MNT`;
});
...
await run("USDY spend card", async () => {
  const decimals = Number(await usdy.decimals().catch(() => 6));
  ...
  await wait(cards.createCard(cardId, deployer.address, deployment.usdyToken, amount, "Smoke USDY Card"));
  await wait(usdy.approve(c.AgentSpendCardVault, amount));
  await wait(cards.topUpCard(cardId, amount));
  await wait(cards.spendCard(cardId, deployer.address, 1n, "smoke spend"));
  return `${await usdy.symbol().catch(() => "USDY")} card funded and spent`;
});
...
await run("USDY privacy vault release", async () => {
  const decimals = Number(await usdy.decimals().catch(() => 6));
  const amount = 1n;
  const balance = await usdy.balanceOf(deployer.address);
  if (balance < amount) return `SKIP balance too low: ${balance.toString()} base units`;
  const commitment = id(`usdy-privacy-${Date.now()}`);
  const nullifier = id(`usdy-release-${Date.now()}`);
  await wait(usdy.approve(c.MantlePrivacyVault, amount));
  await wait(privacy.createTokenIntent(commitment, deployment.usdyToken, amount, "encrypted://arcpay-usdy-smoke"));
  await wait(privacy.releaseIntent(commitment, nullifier, requester.address));
  const intent = await privacy.intents(commitment);
  if (!intent[4]) throw new Error("USDY intent not released");
...
await run("Mantle agent risk oracle", async () => {
