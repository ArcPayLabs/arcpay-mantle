...
if (!process.env.PRIVATE_KEY) {
  throw new Error("PRIVATE_KEY missing. x402 smoke requires a funded Mantle provider signer.");
}
...
try {
  ...
  await run("x402 health", async () => {
    ...
    if (!res.ok || body.service !== "arcpay-mantle-x402") throw new Error("bad health response");
    return `${body.network} chain ${body.chainId}`;
  });

  await run("provider has Mantle funds", async () => {
    const balance = await provider.getBalance(providerWallet.address);
    if (balance < parseEther("0.5")) throw new Error(`low MNT balance ${formatEther(balance)}`);
    return `${providerWallet.address.slice(0, 6)}...${providerWallet.address.slice(-4)} has ${formatEther(balance)} MNT`;
  });
  ...
  await run("register x402 agent", async () => {
    ...
    await wait(registry.registerAgent(agentId, "x402 Mantle Research Agent", endpoint, "x402,research,risk,treasury", parseEther("0.0001")));
  ...
  await run("protected resource returns HTTP 402", async () => {
    ...
    return `${body.accepts[0].amountStt} MNT quoted`;
  });
