...
describe("AgentInvoiceBook", () => {
  it("creates and pays native MNT invoices", async () => {
  ...
  it("creates and pays USDY token invoices", async () => {
    const [issuer, payer] = await ethers.getSigners();
    const token = await ethers.deployContract("MockERC20");
    const book = await ethers.deployContract("AgentInvoiceBook");
    const invoiceId = ethers.id("usdy-invoice-1");
    const amount = 5_000_000n;

    await token.mint(payer.address, amount);
    await book.connect(issuer).createInvoice(invoiceId, payer.address, await token.getAddress(), amount, "invoice://usdy-1");
